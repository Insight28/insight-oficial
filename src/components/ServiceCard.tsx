...
const ServiceCard = ({ title, description, icon, className }: ServiceCardProps) => {
  return (
    <div className={cn(
      "neon-border-red p-8 rounded-lg bg-dark/90 backdrop-blur-sm hover:scale-102 transition-all duration-300 h-full animate-float-slow",
      className
    )}>
      ...
      <p className="text-xl text-gray-200 text-center leading-relaxed">{description}</p>
    </div>
  );
};
